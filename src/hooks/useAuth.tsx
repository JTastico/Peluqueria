...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const login = (newUsername: string, newRole: string, newLocalId?: number | null) => { //
    localStorage.setItem('isAuthenticated', 'true'); //
    localStorage.setItem('userRole', newRole); //
    localStorage.setItem('username', newUsername); //
    if (newLocalId !== undefined && newLocalId !== null) { //
      localStorage.setItem('localId', String(newLocalId)); //
      setLocalId(String(newLocalId)); //
    } else { //
